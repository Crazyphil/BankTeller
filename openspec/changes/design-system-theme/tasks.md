...
- [ ] 11.5 Wrap `OnboardingScreen` in `ScreenShell` with `BrandedTopBar` (logo + wordmark + logout action). Remove ad-hoc logout buttons from individual wizard step content (e.g., `ActivationGuideStep`'s secondary "Logout" button). The `onBack` slot in wizard steps returns to its intended purpose: navigating to the previous wizard step.
...
- [ ] 17.2 Manually verify login screen renders with Private Ledger tokens (warm paper bg, ink-navy primary, Fraunces headline, logo icon + wordmark) in light mode
- [ ] 17.3 Manually verify login screen renders correctly in dark mode (warm charcoal bg, light ink primary, dark logo variant)
- [ ] 17.4 Manually verify onboarding screens inherit the theme and use WizardScaffold (wizard progress indicator, eyebrow, unified back)
- [ ] 17.5 Verify theme toggle: set `localStorage["bankteller-theme"]` to `"dark"` and reload — dark mode persists before login
- [ ] 17.6 Verify font preloading — no FOUT on initial render; fonts load before first composition
- [ ] 17.7 Verify all screens use ScreenShell (no raw Column boilerplate remains)
- [ ] 17.8 Verify avatar colors are theme-derived (no hardcoded hex values remain in avatar logic)
- [ ] 17.9 Verify error presentation follows the 3-context pattern consistently across all screens
- [ ] 17.10 Verify no ad-hoc FilterChip color overrides remain in BankSelection
- [ ] 17.11 Verify favicon appears in browser tab
- [ ] 17.12 Verify logo icon on login screen switches correctly between light/dark variants when theme changes
- [ ] 17.13 Verify logo icon appears on callback success state
- [ ] 17.14 Verify BrandedTopBar appears on dashboard with logo + wordmark + logout action
- [ ] 17.15 Verify BrandedTopBar appears on onboarding screens with logo + wordmark + logout action
- [ ] 17.16 Verify no ad-hoc logout buttons remain in wizard step content (logout is in the top bar)
- [ ] 17.17 Verify BrandedTopBar does NOT appear on login, legal pages, or callback (those screens have no top bar)
