...
**Shared components:**
- Create `ScreenShell` — replaces 6× duplicated screen boilerplate (`Column(fillMaxSize().safeContentPadding().padding(16.dp))`), centers content within `contentMaxWidth`
...
- **New files**: `ui/theme/Color.kt`, `Type.kt`, `Shapes.kt`, `Dimens.kt`, `Theme.kt`, `ui/components/ScreenShell.kt`, `WizardScaffold.kt`, `WizardProgressIndicator.kt`, `DecisionBox.kt` under `app/web/src/commonMain/kotlin/it/kapfer/bankteller/`
- **Modified files**: `App.kt` (replace `MaterialTheme` calls), `Fonts.kt` (add Fraunces/Inter families), `index.html` (font preload tags), `LoginScreen.kt`, `DashboardScreen.kt`, `OnboardingScreen.kt`, `PrivacyScreen.kt`, `TermsScreen.kt`, `CallbackScreen.kt`, `build.gradle.kts` (font resource config if needed)
- **New bundled resources**: Fraunces Variable WOFF2 (~150-250K), Inter Variable WOFF2 (~100-150K) in `composeResources/font/`
