...
#### Scenario: Dashboard uses ScreenShell
- **WHEN** the dashboard renders
- **THEN** the content is wrapped in `ScreenShell`, centered horizontally and constrained to at most 720dp
...
- **THEN** the headline uses an emerald accent (emerald = verified), `CircularProgressIndicator` in `brass` shows during the 2s redirect countdown (auth flow only), and a "BankTeller" wordmark footer anchors the page
